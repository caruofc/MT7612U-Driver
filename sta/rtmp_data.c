...
/* ---Add by shiangf for debug */
	...
	if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1)) {
		...
		if (pRxInfo->MyBss == 0) {
			...
			/* When the p2p-IF up, the STA own address would be set
			   as my_bssid address. If receiving an "encrypted"
			   broadcast packet(its WEP bit as 1) and doesn't
			   match my BSSID, Asic pass to driver with "Decrypted"
			   marked as 0 in pRxInfo.
...
INT STASendPacket(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket)
{
	...
	if (((wdev->AuthMode == Ndis802_11AuthModeWAICERT) ||
		...
		DBGPRINT(RT_DEBUG_OFF, ("%s():Drop packet before port secured!\n", __func__));
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
...
static VOID STA_AMPDU_Frame_Tx(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	...
	while (pTxBlk->TxPacketList.Head) {
...
#endif /* endif */
			/*
			   because the signal format of sounding frmae may be different from
			   normal data frame, which may result in different MFB
			*/
		...
		if ((pMacEntry->isCached)
			...
			RTMPWriteTxWI_Cache(pAd, (TXWI_STRUC *) (&pTxBlk->HeaderBuf[TXINFO_SIZE]),
					    pTxBlk);
		} else {
			RTMPWriteTxWI_Data(pAd, (TXWI_STRUC *) (&pTxBlk->HeaderBuf[TXINFO_SIZE]),
					   pTxBlk);
...
static VOID STA_ARalink_Frame_Tx(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	...
	while (pTxBlk->TxPacketList.Head) {
		...
		if (frameNum == 0) {
			/*
			   For first frame, we need to create the 802.11 header +
			   padding(optional) + RA-AGG-LEN + SNAP Header
			*/
			...
			/*
			   For second aggregated frame, we need create the 802.3 header
			   to headerBuf, because PCI will copy it to SDPtr0.
			*/
			pHeaderBufPtr = &pTxBlk->HeaderBuf[0];
...
static VOID STA_Fragment_Frame_Tx(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk)
{
	...
	if ((pTxBlk->CipherAlg == CIPHER_WEP64)
		...
		/* TKIP: IV[4] + EIV[4] + ICV[4], MIC will be added to TotalPacketLength */
