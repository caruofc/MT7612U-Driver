...
VOID MlmeAssocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	/* check sanity first */
		...
		/*
		   for dhcp,issue ,wpa_supplicant ioctl too fast , at link_up,
		   it will add key before driver remove key.
		*/
		 ...
		 * TGn-5.2.22
		 * AP may not set this bit on, so we set this following dat settings.
		 */
		...
		DBGPRINT(RT_DEBUG_OFF, ("%s ASSOC - Send ASSOC request...\n", __func__));
		...
		if ((pAd->MlmeAux.HtCapabilityLen > 0)
			...
			*(USHORT *) (&HtCapabilityTmp.ExtHtCapInfo) =
			...
			if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
				...
				/*
				   Disable beamform capability in Associate Request with 3x3 AP to
				   avoid throughput drop issue.
		...
		/*
		   add Ralink proprietary IE to inform AP this STA is going to use
		   AGGREGATION or PIGGY-BACK+AGGREGATION
		...
		*/
		if (pAd->CommonCfg.bAggregationCapable) {
			if ((pAd->CommonCfg.bPiggyBackCapable)
				...
				ULONG TmpLen;
				UCHAR RalinkIe[9] = { IE_VENDOR_SPECIFIC, 7, 0x00,
					0x0c, 0x43, 0x03, 0x00, 0x00, 0x00 };
...
/* #endif */
			...
			if (((pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP & 0x7F) !=
				...
				|| (pAd->StaCfg.wpa_supplicant_info.bRSN_IE_FromWpaSupplicant
				== FALSE))
				/* #endif */
#endif /* WPA_SUPPLICANT_SUPPORT */
			{
...
#ifdef VHT_TXBF_SUPPORT
					/*
					   Disable beamform capability in Associate Request
					   with 3x3 AP to avoid throughput drop issue.
					   MT76x2 only supports up to 2x2 sounding feedback.
					*/
					Idx =
					    BssTableSearch(&pAd->ScanTab, pAd->MlmeAux.Bssid,
...
VOID MlmeDisassocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pDisassocReq->Addr,
		pAd->CurrentAddress, pDisassocReq->Addr);
	MakeOutgoingFrame(pOutBuffer, &FrameLen,
	...
	/*
	   ...
	   Driver has to send deauth to Instance AP, but Buffalo(N) needs to send disassoc to
	   reset Authenticator's state machine.
...
VOID Cls3errAction(RTMP_ADAPTER *pAd, UCHAR *pAddr)
{
	...
	MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pAddr, pAd->CurrentAddress,
	...
	/*
	   To patch Instance and Buffalo(N) AP.
	   Driver has to send deauth to Instance AP, but Buffalo(N) needs to send disassoc
	   to reset Authenticator's state machine.
	   Therefore, we send both of them.
	*/
	pDisassocHdr = (PHEADER_802_11) pOutBuffer;
	pDisassocHdr->FC.SubType = SUBTYPE_DEAUTH;
