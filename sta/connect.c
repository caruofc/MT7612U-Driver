...
{                                                                                       \
	...
	NdisMoveMemory(&(_pAd)->CommonCfg.APQosCapability, &(_pAd)->MlmeAux.APQosCapability,\
				  ...
				  IN STATE_MACHINE * S, IN MLME_QUEUE_ELEM * Elem)
{
	...
	case CNTL_WAIT_OID_LIST_SCAN:
		if (Elem->MsgType == MT2_SCAN_CONF) {
			...
			/*
			   AP sent a 2040Coexistence mgmt frame, then station perform
			   a scan, and then send back the respone.
			*/
...
#endif /* RT_CFG80211_SUPPORT */
			/* Resume TxRing after SCANNING complete. We hope the out-of-service time */
			/* won't be too long to let upper layer time-out the waiting frames */
			RTMPResumeMsduTransmission(pAd);
			pAd->Mlme.CntlMachine.CurrState = CNTL_IDLE;
			BssTableSsidSort(pAd, &pAd->MlmeAux.SsidBssTab,
...
/* Disable scanning when ATE is running. */
	if (ATE_ON(pAd)) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: ate on!!!\n", __func__));
#ifdef RT_CFG80211_SUPPORT
...
VOID CntlOidSsidProc(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (INFRA_ON(pAd) &&
		...
		/*
		   ...
		   RT61 may lost SSID, and not connect to NDTEST_WEP_AP2 and will connect
		   to NDTEST_WEP_AP2 by Autoreconnect
		...
		if (!SSID_EQUAL
		    (pAd->CommonCfg.Ssid, pAd->CommonCfg.SsidLen, pAd->MlmeAux.Ssid,
		     pAd->MlmeAux.SsidLen)) {
			/*
			   Different SSID means not Roaming case, so we let LinkDown()
			   to Indicate a disconnect event.
			*/
	...
	if (Elem->MsgType == MT2_JOIN_CONF) {
		...
		if (Reason == MLME_SUCCESS) {
			...
			/* 2. joined a new INFRA network, start from authentication */
				...
				{
					/*
					   either Ndis802_11AuthModeShared or
...
VOID CntlWaitAuthProc(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (Elem->MsgType == MT2_AUTH_CONF) {
		...
		if (Reason == MLME_SUCCESS) {
			...
			/*
			   This fail may because of the AP already
			   keep us in its MAC table without
			   ageing-out. The previous authentication
			   attempt must have let it remove us.
			   so try Authentication again may help.
			   For D-Link DWL-900AP+ compatibility.
			*/
...
VOID CntlWaitAssocProc(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (Elem->MsgType == MT2_ASSOC_CONF) {
		NdisMoveMemory(&Reason, Elem->Msg, sizeof(USHORT));
		if (Reason == MLME_SUCCESS) {
			/* Due to LinkUp() consume too much time, it might be too late
			 * to notify wpa_supplicant that driver is associated.
			 * It might cause EAPOL Key exchange fail with Marvell AP.
			 * So call LinkUp() after send event to inform kernel.
			 */
#ifdef RT_CFG80211_SUPPORT
			/* moved from assoc.c */
			{
...
/* wdev->bw = pAd->CommonCfg.RegTransmitSetting.field.BW; */
	...
	/*
	   Note: Move this "Beacon Rx time update" upward earlier in order
	   to solve the problem of timing issue that we saw right after Link UP,
	   the driver could trigger Beacon Loss > 10000ms falsely.
	*/
	/* NdisGetSystemUpTime(&Now); */
...
/*			RTMPWPARemoveAllKeys(pAd);*/
		...
		/*
		   ...
		   We cannot move this to PeerBeaconAtJoinAction because PeerBeaconAtJoinAction
		   wouldn't be invoked in roaming case.
		 */
		...
		/*
		   ...
		   2. When security of Marvell WPS AP is OPEN/NONE, Marvell AP
		   will send EAP-Req(ID) to STA immediately.
		   STA needs to receive this EAP-Req(ID) on time, because Marvell
		   AP will not send again after STA sends EAPOL-Start.
		 */
	...
	/*
	   if 1. Legacy AP WMM on,  or 2. 11n AP, AMPDU disable.
...
/* pAd->MlmeAux.InfraChannel = 0; */
	...
	if (pAd->Mlme.bStartMcc == TRUE) {
		...
		/* yiwei : PairwiseKey seems not clear in MacTableDeleteEntry() */
		if (((pMacEntry->PairwiseKey.KeyLen == LEN_TK) &&
			       ...
			       IN UINT8 BandWidth, IN UCHAR PriCh, IN UCHAR ExtraCh)
{
	...
	*pBwFallBack = 0;
	...
	if (pAd->StaActive.SupportedPhyInfo.bVhtEnable == TRUE &&
	    pAd->StaActive.SupportedPhyInfo.vht_bw == VHT_BW_80) {
		rf_bw = BW_80;
		/*for sta 80M and GC concurrent mode*/
	...
	if (IS_MT76x2(pAd)) {
		...
		/* MAC will be disabled by calibration,
		   so it should finish before any packet arrival */
