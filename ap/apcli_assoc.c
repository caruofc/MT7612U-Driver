		...
		*pCurrState = APCLI_ASSOC_IDLE;
		...
		if (pAd->CommonCfg.bAggregationCapable) {
			...
			if ((pAd->CommonCfg.bPiggyBackCapable)
				...
				ULONG TmpLen;
				UCHAR RalinkIe[9] = {
				    IE_VENDOR_SPECIFIC, 7, 0x00, 0x0c, 0x43, 0x03, 0x00, 0x00,
					0x00
				};
			...
			if (apcli_entry->MlmeAux.APRalinkIe & 0x00000001) {
				ULONG TmpLen;
				UCHAR RalinkIe[9] = {
				    IE_VENDOR_SPECIFIC, 7, 0x00, 0x0c, 0x43, 0x01, 0x00, 0x00,
					0x00
				};
				MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 9, RalinkIe,
						  END_OF_ARGS);
			...
			if (CliIdx == 0xFF)
#endif /* MAC_REPEATER_SUPPORT */
				{
					...
					if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode)
						...
						DBGPRINT(RT_DEBUG_TRACE,
							 ("There is vht le at Assoc Rsp ifIndex=%d vht_cap_len=%d\n",
