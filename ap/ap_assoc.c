...
/*		SendSingalToDaemon(SIGUSR2, pObj->IappPid); */
		...
		if (TRUE) {	/*CFG_TODO */
			...
			pNetDev = RTMP_CFG80211_FindVifEntry_ByMacAddr(pAd,
					ie_list->Addr1);
			...
			if ((pNetDev != NULL) &&
				...
				DBGPRINT(RT_DEBUG_TRACE,
					("GO NOITFY THE CLIENT ASSOCIATED\n"));
			...
			if ((pNetDev != NULL) &&
				...
				DBGPRINT(RT_DEBUG_TRACE,
					("AP NOITFY THE CLIENT ASSOCIATED\n"));
				CFG80211OS_NewSta(pNetDev, ie_list->Addr2, (PUCHAR) Elem->Msg,
						  Elem->MsgLen);
			} else
		...
		if ((pEntry->AuthMode == Ndis802_11AuthModeWPAPSK) ||
			...
			/*
			   In WPA-PSK mode,
			   If Association Request of station has RSN/SSN, WPS AP Must Not send
			   EAP-Request/Identity to station
...
#ifdef DOT1X_SUPPORT
		/*else if (isReassoc && */
		else if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2) && ((pPmkid = WPA_ExtractSuiteFromRSNIE(ie_list->RSN_IE, ie_list->RSNIE_Len, PMKID_LIST, &pmkid_count)) != NULL)) {	/* Key cache */
			INT CacheIdx;

			CacheIdx = RTMPSearchPMKIDCache(pAd, pEntry->apidx, pEntry->Addr);
			if ((CacheIdx != -1)
			    &&
			    (RTMPEqualMemory
