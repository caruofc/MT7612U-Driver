...
VOID MacTableMaintenance(RTMP_ADAPTER *pAd)
{
	...
	for (i = startWcid; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (pEntry->NoDataIdleCount >= pEntry->StaIdleTimeout
			...
			(pEntry->NoDataIdleCount >= MAC_TABLE_MIN_AGEOUT_TIME)))
#ifdef PS_ENTRY_MAITENANCE
	...
	For 2.4GHz,
		...
		where
			...
			OTi  is member i of the set of channels that comparises all channels that are members of the
				channel set C that were listed once in the Channel List fields of 20/40 BSS
				Intolerant Channel Report elements receved during the previous X seconds
				and all channels that are members of the channel set C and that are the
				primary operating channel of at least one 20/40 MHz
				BSS that were detected within the AP's BSA during the previous X seconds.
			C	is the set of all channels that are allowed operating channels within the
				current operational regulatory domain and whose center frequency falls
				within the 40 MHz affected channel
			...
			"==" means that the values on either side of the "==" are to be tested for
			equaliy with a resulting Boolean value.
		...
		if (index < pAd->ChannelListNum) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				  ...
				  ((pAd->CommonCfg.RegTransmitSetting.field.EXTCHA ==
				    EXTCHA_ABOVE) ? "ABOVE" : "BELOW"), pCoexChRange->primaryCh,
	...
	{			/* For 2.4GHz band */
		...
		if (index < pAd->ChannelListNum) {
			...
			if (cntrCh) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					  ("2.4GHz: Found CtrlCh idx(%d) from the ChList, ExtCh=%s, PrimaryCh=[Idx:%d, CH:%d], SecondaryCh=[Idx:%d, CH:%d], effected Ch=[CH:%d~CH:%d]!\n",
					  index,
					  ((pAd->CommonCfg.RegTransmitSetting.field.EXTCHA ==
