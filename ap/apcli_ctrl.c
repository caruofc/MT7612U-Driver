			       ...
			       IN STATE_MACHINE * Sm, OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(Sm, APCLI_CTRL_DISCONNECTED, APCLI_CTRL_TRIAL_CONNECT,
			      (STATE_MACHINE_FUNC) ApCliCtrlTrialConnectAction);
	StateMachineSetAction(Sm, APCLI_CTRL_TRIAL_TRIGGERED, APCLI_CTRL_JOIN_REQ_TIMEOUT,
	...
	for (i = 0; i < MAX_APCLI_NUM; i++) {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("woody init pApCliEntry->TrialConnectTimer\n"));
		RTMPInitTimer(pAd,
					  ...
					  IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((pMacEntry->PortSecured == WPA_802_1X_PORT_SECURED)
		...
		/* MacTableDeleteEntry(pAd, pApCliEntry->MacTabWCID,
		APCLI_ROOT_BSSID_GET(pAd, pApCliEntry->MacTabWCID)); */
		DBGPRINT(RT_DEBUG_TRACE,
...
static VOID ApCliCtrlJoinReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("(%s) Start Probe Req.\n", __func__));
	...
	if ((pWpsCtrl->WscConfMode != WSC_DISABLE) && (pWpsCtrl->bWscTrigger == TRUE)) {
		...
		if (pWpsCtrl->WscMode == 1) {	/* PIN */
			...
			old_conf_mode = pWpsCtrl->WscConfMode;
			...
			if ((RootApChannel != channel) ||
				...
				/*
				   ApStop will call WscStop, we need to reset WscConfMode,
...
static VOID ApCliCtrlProbeRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (Status == MLME_SUCCESS) {
		...
		*pCurrState = APCLI_CTRL_AUTH;
		...
		AuthReq.Timeout = APCLI_AUTH_TIMEOUT;
#ifdef MAC_REPEATER_SUPPORT
...
#endif /* APCLI_CONNECTION_TRIAL */
			Timeout = ((pApCliEntry->AssocReqCnt <= 2) ?
				APCLI_ASSOC_LITTLE_TIMEOUT : APCLI_ASSOC_TIMEOUT);
			AssocParmFill(pAd, &AssocReq, pApCliEntry->MlmeAux.Bssid,
				      pApCliEntry->MlmeAux.CapabilityInfo, Timeout, 5);
#ifdef APCLI_CONNECTION_TRIAL
		}
#endif /* APCLI_CONNECTION_TRIAL */
...
static VOID ApCliCtrlAuth2RspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (CliIdx != 0xFF)
		...
		pCurrState = &pApCliEntry->CtrlCurrState;
	if (Status == MLME_SUCCESS) {
		DBGPRINT(RT_DEBUG_OFF, ("(%s) Auth2 Rsp Success.\n", __func__));
		*pCurrState = APCLI_CTRL_ASSOC;
		...
		AssocParmFill(pAd, &AssocReq, pApCliEntry->MlmeAux.Bssid,
			      pApCliEntry->MlmeAux.CapabilityInfo, APCLI_ASSOC_TIMEOUT, 5);
...
static VOID ApCliCtrlAssocReqTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	*pCurrState = APCLI_CTRL_ASSOC;
	...
	AssocParmFill(pAd, &AssocReq, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid,
		      pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.CapabilityInfo, Timeout, 5);
...
static VOID ApCliCtrlDeAuthAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("(%s) MLME Request Disconnect.\n", __func__));
