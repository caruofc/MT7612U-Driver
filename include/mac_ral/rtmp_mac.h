...
#define MAC_BSSID1_DW0		0x1098
#define MAC_BSSID1_DW1		0x109C
#define BSSID_EN            0x80000000
...
typedef union _RF_CSR_CFG1_STRUC {
	struct {
		...
		UINT32 RFGap:5;	/* Gap between BB_CONTROL_RF and RF_LE. 0: 3 system clock
						cycle (37.5usec) 1: 5 system clock cycle (62.5usec) */
		UINT32 RegIdAndContent:24;	/* Register value to program into BBP */
	} field;
	UINT32 word;
} RF_CSR_CFG1_STRUC;
...
typedef union _LED_CFG_STRUC {
	struct {
		...
		UINT32 RLedMode:2;	/* red Led Mode    0: off1: blinking upon TX
...
typedef union _IFS_SLOT_CFG_STRUC {
	struct {
		...
		UINT32 OfdmXifsTime:4;	/*OFDM SIFS. unit 1us. Applied after OFDM RX when
								MAC doesn't reference BBP signal BBRXEND */
...
typedef union _TX_TIMEOUT_CFG_STRUC {
	struct {
		UINT32 rsv2:8;
		UINT32 TxopTimeout:8;	/*TXOP timeout value for TXOP truncation.
								It is recommended that (SLOT_TIME) >
								(TX_OP_TIMEOUT) > (RX_ACK_TIMEOUT) */
...
typedef union _TX_RTY_CFG_STRUC {
	struct {
		...
		UINT32 AggRtyMode:1;	/* Aggregate MPDU retry mode.0:expired by retry limit,
								1: expired by mpdu life timer */
		UINT32 NonAggRtyMode:1;	/* Non-Aggregate MPDU retry mode.0:expired by retry limit,
								1: expired by mpdu life timer */
...
typedef union _TX_LINK_CFG_STRUCT {
	struct {
		...
		UINT32 RemoteUMFSEnable:1;	/*  remote unsolicit  MFB enable.
...
typedef union _PROT_CFG_STRUC {
	struct {
		...
		UINT32 ProtectNav:2;	/*TXOP protection type for CCK TX. 0:None,
								1:ShortNAVprotect,  2:LongNAVProtect, 3:rsv */
		UINT32 ProtectCtrl:2;	/*Protection control frame type for CCK TX.
								1:RTS/CTS, 2:CTS-to-self, 0:None, 3:rsv */
...
typedef union _TX_AGG_CNT_STRUC {
	struct {
		UINT16 cnt16_31; /* the count of aggregation size = x + 1 */
		UINT16 cnt0_15;	/* the count of aggregation size = x */
	} field;
	UINT32 word;
} TX_AGG_CNT_STRUC;
#else
