...
#define ACTIVE_SCAN_TIME	    60
#define PASSIVE_SCAN_TIME	    110
...
#define CAP_GENERATE(ess, ibss, priv, s_pre, s_slot, spectrum)  (((ess) ? 0x0001 : 0x0000) |   \
	((ibss) ? 0x0002 : 0x0000) | ((priv) ? 0x0010 : 0x0000) | ((s_pre) ? 0x0020 : 0x0000) | \
...
* If one Event A delete or created, or if Event B is detected or not detected,
* STA should send 2040BSSCoexistence to AP.
*/
...
typedef struct {
	...
	USHORT AmsduEnable:1;	/* Enable to transmit A-MSDU. Suggest disable.
				* We should use A-MPDU to gain best benifit of 802.11n
				*/
	USHORT RxSTBC:2;
	USHORT TxSTBC:1;
...
#ifdef RT_BIG_ENDIAN
	UCHAR RecomWidth:1;
	UCHAR ExtChanOffset:2;	/* Please not the difference with following
				* UCHAR NewExtChannelOffset;from 802.11n
				*/
	UCHAR MpduDensity:3;
	...
	UCHAR MaxRAmpduFactor:2;
	UCHAR MpduDensity:3;
	UCHAR ExtChanOffset:2;	/* Please not the difference with following UCHAR
				* NewExtChannelOffset; from 802.11n
				*/
	UCHAR RecomWidth:1;
#endif				/* endif */
...
typedef struct _BSS_ENTRY {
	...
	UCHAR Channel;
	UCHAR CentralChannel;	/* Store the wide-band central channel for 40MHz.
				* used in 40MHz AP. Or this is the same as Channel.
				*/
	...
	UCHAR AddHtInfoLen;
	EXT_CAP_INFO_ELEMENT ExtCapInfo; /* this is the extended capibility IE appreed in
					* MGMT frames. Doesn't need to update once set in Init.
					*/
	UCHAR NewExtChanOffset;
	CHAR Rssi;
...
#ifdef WSC_INCLUDED
	UCHAR WpsAP;		/* 0x00: not support WPS, 0x01: support normal WPS,
				* 0x02: support Ralink auto WPS, 0x04: support WAC AP
				*/
...
typedef struct _MLME_ASSOC_REQ_STRUCT {
	...
	USHORT ListenIntv;
	ULONG Timeout;
} MLME_ASSOC_REQ_STRUCT, *PMLME_ASSOC_REQ_STRUCT,
MLME_REASSOC_REQ_STRUCT, *PMLME_REASSOC_REQ_STRUCT;
