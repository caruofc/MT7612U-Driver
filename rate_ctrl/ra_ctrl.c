...
/* Item No.   Mode   Curr-MCS   TrainUp   TrainDown		 Mode- Bit0: STBC,
 * Bit1: Short GI, Bit4~6: Mode(0:CCK, 1:OFDM, 2:HT Mix, 3:HT GF)*/
...
/* Item No.   Mode   Curr-MCS   TrainUp   TrainDown		 Mode- Bit0: STBC,
 * Bit1: Short GI, Bit4~6: Mode(0:CCK, 1:OFDM, 2:HT Mix, 3:HT GF)*/
	0x0a, 0x06, 0, 0, 0,	/* Initial used item after association */
	0x00, 0x00, 0, 40, 101,
	0x01, 0x00, 1, 40, 50,
	0x02, 0x00, 2, 35, 45,
	0x03, 0x00, 3, 20, 45,
...
/* Item No.   Mode   Curr-MCS   TrainUp   TrainDown		 Mode- Bit0: STBC,
 * Bit1: Short GI, Bit4~6: Mode(0:CCK, 1:OFDM, 2:HT Mix, 3:HT GF)*/
	0x08, 0x05, 0, 0, 0,	/* Initial used item after association */
	0x00, 0x10, 0, 20, 101,
	0x01, 0x10, 1, 20, 35,
	0x02, 0x10, 2, 20, 35,
	0x03, 0x10, 3, 16, 35,
...
if (!IS_ENTRY_TDLS(pEntry))
#endif /* CFG_TDLS_SUPPORT */
		{
			...
			if (pEntry->MaxHTPhyMode.field.BW == BW_20 || wdev->bw == BW_20)
	...
	do {
...
#ifdef WFA_VHT_PF
			/* TODO: shiang, add for Realtek behavior when run in BW signaling mode test
			 * and we are the testbed! */
			/* TODO: add at 11/15! */
		...
		if ((pAd->OpMode == OPMODE_STA) && ADHOC_ON(pAd)) {
			...
			if (WMODE_CAP_N(pAd->CommonCfg.PhyMode)
				...
				|| (pAd->Antenna.field.TxPath == 1))) {	/* 11N 1S Adhoc */
				...
				|| (pAd->Antenna.field.TxPath == 2))) {	/* 11N 2S Adhoc */
#ifdef AGS_SUPPORT
				if (SUPPORT_AGS(pAd)) {
...
#ifdef AGS_SUPPORT
		/* only for station */
		if (SUPPORT_AGS(pAd) && (pEntry->HTCapability.MCSSet[0] != 0x00)
			&& (pEntry->HTCapability.MCSSet[1] != 0x00)
			&& (pEntry->HTCapability.MCSSet[2] != 0x00)
			&& (pAd->CommonCfg.TxStream == 3)) {	/* 11N 3S */
			*ppTable = AGS3x3HTRateTable;
			break;
		}
#endif /* AGS_SUPPORT */

		/*else if ((pAd->StaActive.SupRateLen + pAd->StaActive.ExtRateLen == 12)
		 * && (pAd->StaActive.SupportedPhyInfo.MCSSet[0] == 0xff) && */
		...
		if ((pEntry->SupportRateMode & (SUPPORT_OFDM_MODE)) &&
		    ...
		    (((pAd->Antenna.field.TxPath == 3)
		    && (pEntry->HTCapability.MCSSet[2] == 0x00))
		    || (pAd->CommonCfg.TxStream == 2))) {	/* 11BGN 2S AP */
#ifdef DOT11_VHT_AC
			if (CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_2G_256QAM_CAPABLE)
			    && (pAd->LatchRfRegs.Channel <= 14)) {
				if (pEntry->MaxHTPhyMode.field.BW == BW_20)
		...
		/*else if ((pAd->StaActive.SupportedPhyInfo.MCSSet[0] == 0xff)
		 *&& ((pAd->StaActive.SupportedPhyInfo.MCSSet[1] == 0x00)
		...
		if ((pAd->CommonCfg.TxStream == 3)
			...
			if (pEntry->HTCapability.MCSSet[2] == 0x00) {	/* Only support 2SS */
				if (pEntry->RateLen > 0) {
#ifdef NEW_RATE_ADAPT_SUPPORT
					if (pAd->rateAlg == RATE_ALG_GRP)
						*ppTable = RateSwitchTableAdapt11N2S;
		...
		/*else if ((pAd->StaActive.SupRateLen + pAd->StaActive.ExtRateLen > 8)
		 *&& (pAd->StaActive.SupportedPhyInfo.MCSSet[0] == 0)
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			/*else if ((pAd->StaActive.SupportedPhyInfo.MCSSet[0] == 0)
			 *&& (pAd->StaActive.SupportedPhyInfo.MCSSet[1] == 0)) */
			if ((pEntry->HTCapability.MCSSet[0] == 0)
				&& (pEntry->HTCapability.MCSSet[1] == 0)) {	/* Legacy mode */
		       ...
		       IN PMAC_TABLE_ENTRY pEntry, IN CHAR mcs[], IN CHAR Rssi, IN CHAR RssiOffset)
{
	...
	if (pTable == RateTableVht2S || pTable == RateTableVht2S_BW20
		...
		if (mcs[15] >= 0 && (Rssi >= (-70 + RssiOffset)) && (pEntry->SupportVHTMCS[MCS_15]))
		   ...
		   || (pTable == RateTableVht1S_2G_BW40)) {	/*  VHT mode with 1SS */
...
#ifdef MT85XX
	/* HE BDP 8507 plaform crash issue:kernel stack buffer overflow*/
	static UCHAR SupportedRates[MAX_LEN_OF_SUPPORTED_RATES];
#else
	 UCHAR SupportedRates[MAX_LEN_OF_SUPPORTED_RATES];
#endif

	NdisZeroMemory(SupportedRates, sizeof(SupportedRates));
...
INT Set_RateAlg_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s: Set Alg = %ld\n", __func__, ra_alg));
	return TRUE;
}
