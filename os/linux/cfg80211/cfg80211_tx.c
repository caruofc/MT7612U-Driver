	...
	if (mgmtFrame->pBuf != NULL) {
...
#endif /* endif */
		{
			...
			if (ieee80211_is_probe_resp(mgmt->frame_control)) {
				...
				CFG80211_SyncPacketWmmIe(pAd, (VOID *)(mgmtFrame->pBuf +
							      offset),
							 mgmtFrame->len -
							 offset);
#endif /* endif */
	...
	if (ieee80211_is_action(mgmt->frame_control)) {
		...
		if ((pP2PFrame->p80211Header.FC.SubType == SUBTYPE_ACTION) &&
			...
			case GO_NEGOCIATION_CONFIRM:
				DBGPRINT(RT_DEBUG_ERROR, ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> ",
					  __func__, PRINT_MAC(mgmt->sa)));
				DBGPRINT(RT_DEBUG_ERROR, ("(%02X:%02X:%02X:%02X:%02X:%02X) ",
					  PRINT_MAC(mgmt->da)));
				DBGPRINT(RT_DEBUG_ERROR,
					("GO_NEGOTIATION_CONFIRM IsNeedTxStatus=TRUE %d\n",
					  pAd->LatchRfRegs.Channel));
			...
			case P2P_INVITE_RSP:
				...
				DBGPRINT(RT_DEBUG_ERROR, ("(%02X:%02X:%02X:%02X:%02X:%02X) ",
					  PRINT_MAC(mgmt->da)));
				DBGPRINT(RT_DEBUG_ERROR,
					("P2P_INVITE_RSP IsNeedTxStatus=TRUE %d\n",
					  pAd->LatchRfRegs.Channel));
			...
			case P2P_DEV_DIS_REQ:
				...
				DBGPRINT(RT_DEBUG_ERROR, ("(%02X:%02X:%02X:%02X:%02X:%02X) ",
					  PRINT_MAC(mgmt->da)));
				DBGPRINT(RT_DEBUG_ERROR,
					("P2P_DEV_DIS_REQ IsNeedTxStatus=TRUE %d\n",
					  pAd->LatchRfRegs.Channel));
			...
			default:
				IsNeedTxStatus = NeedTxStatus;
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s unhandle P2P action subtype\n", __func__));
			}
		} else if (
			(pP2PFrame->p80211Header.FC.SubType == SUBTYPE_ACTION) &&
			(pP2PFrame->Category == CATEGORY_PUBLIC)) {
			DBGPRINT(RT_DEBUG_ERROR,
				("%s: (%pM) -> (%pM) %d, Seq:%d\n",
				__func__,
				...
				mgmt->da,
				pAd->LatchRfRegs.Channel,
				pP2PFrame->p80211Header.Sequence));
