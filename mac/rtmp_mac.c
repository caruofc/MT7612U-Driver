...
VOID RTMPWriteTxWI(RTMP_ADAPTER *pAd, TXWI_STRUC *pOutTxWI, BOOLEAN FRAG,
		   BOOLEAN CFACK, BOOLEAN InsTimestamp, BOOLEAN AMPDU,
		   BOOLEAN Ack, BOOLEAN NSeq,	/* HW new a sequence. */
...
#if defined(MT76XX_BTCOEX_SUPPORT) && defined(JUNGLE_SUPPORT)
...
#ifdef TXBF_SUPPORT
		if (pTxBlk->TxSndgPkt > SNDG_TYPE_DISABLE)
			/* Reserve larger TXOP to prevent sounding packet from collision */
			txwi_n->txop = IFS_BACKOFF;
		else
#endif /* TXBF_SUPPORT */
...
RTMP_REG_PAIR MACRegTable[] = {
	...
	{TX_LINK_CFG, 0x1020}
	,			/* Gary,2006-08-23 */
	/*{TX_TIMEOUT_CFG,      0x00182090}, */
	/*CCK has some problem. So increase timieout value. 2006-10-09 MArvek RT */
