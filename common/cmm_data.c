		...
		if (pEntry && (pEntry->NoBADataCountDown == 0) && IS_HT_STA(pEntry)) {
			...
			if (IS_ENTRY_APCLI(pEntry)) {
				if (pEntry->apidx < MAX_APCLI_NUM) {
					if (pAd->ApCfg.ApCliTab[pEntry->apidx].
						MlmeAux.APRalinkIe != 0)
						isRalink = TRUE;
				}
			}
#endif /* APCLI_SUPPORT */
			      ...
			      IN UCHAR QueIdx, IN UCHAR *pData, IN UINT Length)
{
	...
	do {
		...
		if ((FreeNum > 0)) {
			...
			/* We need to reserve space for rtmp hardware header. i.e.,
			*  TxWI for RT2860 and TxInfo+TxWI for RT2870
			*/
			NdisZeroMemory(&rtmpHwHdr, hw_len);
			Status = RTMPAllocateNdisPacket(pAd, &pPacket,
				(PUCHAR) & rtmpHwHdr[0], hw_len, pData, Length);
...
/*		(FlgIsDeltsFrame == 0))*/
	...
	{
		/* DuplicatePkt =
		*  DuplicatePacket(get_netdev_from_bssid(pAd, pMacEntry->apidx),
		*  pPacket, pMacEntry->apidx);
		*/
		ULONG IrqFlags = 0;
				     ...
				     IN UCHAR QueIdx, IN PNDIS_PACKET pPacket)
{
	...
	if (pHeader_802_11->FC.Type == FC_TYPE_CNTL) {	/* must be PS-POLL */
		...
		if (pHeader_802_11->Addr1[0] & 0x01) {	/* MULTICAST, BROADCAST */
...
#endif /* SOFT_SOUNDING */
			{
				...
				/* WCNCR00046134 GO Case:
				*  OFDM Mgmt frame duration set to 60us for IOT
				*/
				...
				if ((pHeader_802_11->FC.SubType == SUBTYPE_PROBE_RSP)
					...
					/* Disable ACK to prevent retry 0x1f for Probe Response */
					bAckRequired = FALSE;
#ifdef CONFIG_AP_SUPPORT
#ifdef SPECIFIC_TX_POWER_SUPPORT
					...
					(pAd->CommonCfg.MlmeTransmit.field.MCS == RATE_1) */
					   ...
					   && (pHeader_802_11->FC.Type == FC_TYPE_MGMT)) {
					/* Disable ACK to prevent retry 0x1f for Probe Request */
					bAckRequired = FALSE;
				} else if ((pHeader_802_11->FC.SubType == SUBTYPE_DEAUTH) &&
					   (MacTableLookup(pAd, pHeader_802_11->Addr1) == NULL)) {
					/* Disable ACK to prevent retry 0x1f for Deauth */
					bAckRequired = FALSE;
				}
			}
		}
	...
	if (pAd->cfg80211_ctrl.IsNeedTxStatus) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			...
			__func__, TXStatus->WCID, TXStatus->MgmtID,
			TXStatus->Seq, TXStatus->Timeout));
	...
	if (((pHeader_802_11->FC.Type == FC_TYPE_DATA)
		...
		DBGPRINT(RT_DEBUG_OFF,
			...
			(transmit->field.MCS >> 4) + 1,
			(transmit->field.MCS & 0xf)));
	} else
#endif /* SOFT_SOUNDING */
	{
		RTMPWriteTxWI(pAd, pFirstTxWI, FALSE, FALSE, bInsertTimestamp, FALSE, bAckRequired,
...
Basically, we divide the packets into four types, Broadcast/Multicast,
11N Rate(AMPDU, AMSDU, Normal), B/G Rate(ARALINK, Normal), Specific Packet Type.
Following show the classification rule and policy for each kinds of packets.
Classification Rule=>
	...
	11N Rate : If peer support HT
		...
		(2).AMSDU  -- If AMSDU is capable for both peer and ourself.
			      *). AMSDU can embedded in a AMPDU, but now we
				didn't support it.
		(3).Normal -- Other packets which send as 11n rate.

	B/G Rate : If peer is b/g only.
		(1).ARALINK-- If both of peer/us supprot Ralink proprietary
			      Aggregation and the TxRate is large than RATE_6
		(2).Normal -- Other packets which send as b/g rate.
	Fragment:
		The packet must be unicast, NOT A-RALINK, NOT A-MSDU, NOT 11n,
		then can consider about fragment.
...
Classified Packet Handle Rule=>
	...
	Specific :
		Basically, for specific packet, we should handle it specifically,
		but now all specific packets are use the same policy to handle it.
		Force LowRate,  pTxBlk->bForceLowRate = TRUE;
...
static UCHAR TxPktClassification(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket, TX_BLK *pTxBlk)
{
	...
	/* It's a specific packet need to force low rate,
	* i.e., bDHCPFrame, bEAPOLFrame, bWAIFrame
	*/
	...
	if (pTxBlk->Wcid == MCAST_WCID) {
...

		{
...
#endif /* P2P_SUPPORT || RT_CFG80211_P2P_SUPPORT */
			{
				...
				if (IS_ENTRY_APCLI(pMacEntry)) {
					...
					{
						/* For each tx packet, update our MAT convert
						*  engine databases.
						*/
						apCliPkt =
		...
		if (pTxBlk->TxFrameType == TX_LEGACY_FRAME) {
...
#ifdef RT_CFG80211_SUPPORT
				if (pNetDev &&
					(RT_CFG80211_GET_IF_TYPE(pNetDev) ==
...
#endif /* WAPI_SUPPORT */
				/* Modify the WMM bit for ICV issue.
				*  If we have a packet with EOSP field need to set as 1,
				*  how to handle it?
				*/
				...
				if (RTMP_GET_PACKET_LOWRATE(pPacket) &&
				((pAd->ApCfg.ApCliTab[0].CtrlCurrState ==
			...
			if ((IS_HT_RATE(pMacEntry) == FALSE) &&
				...
				/* Currently piggy-back only support when
				* peer is operate in b/g mode.
				*/
				      ...
				      IN NDIS_PACKET *pPacket, IN TX_BLK *pTxBlk)
{
	...
	if ((pTxBlk->TxFrameType == TX_RALINK_FRAME) && (pTxBlk->TxPacketList.Number == 2))
		return FALSE;
...
#ifdef CONFIG_AP_SUPPORT
		/* CFG_TODO */
		if ((MAC_ADDR_EQUAL(GET_OS_PKT_DATAPTR(pTxBlk->pPacket),
...
/*
	...
	========================================================================
*/
VOID RTMPDeQueuePacket(IN RTMP_ADAPTER *pAd, IN BOOLEAN bIntContext,
	IN UCHAR QIdx, IN INT Max_Tx_Packets)
{
	...
	if (ATE_ON(pAd)) {
		if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) {
			...
			DBGPRINT(RT_DEBUG_OFF,
				("%s clear BSS_SCAN_IN_PROGRESS in ATE\n",
				__func__));
		}
	}
#endif /* RALINK_ATE */

	for (qiter = sQIdx; qiter >= eQIdx; --qiter) {
		QueIdx = (UINT32) qiter;
		Count = 0;
		...
		while (1) {
...
#endif /* endif */
			pTxBlk = &TxBlk;
			NdisZeroMemory(pTxBlk, sizeof(*pTxBlk));
			...
			if (RTMP_GET_PACKET_OPMODE(pPacket)) {
				...
				if (RTMP_GET_PACKET_MGMT_PKT(pPacket) == 1) {
					...
					status = MlmeHardTransmitMgmtRing(pAd,
						QueIdx, pPacket);
...
#define ENTRY_RETRY_INTERVAL	(100 * OS_HZ / 1000)
			...
			hasTxDesc = RTMP_HAS_ENOUGH_FREE_DESC(pAd, pTxBlk,
			...
			if (pTxBlk->TxFrameType & (TX_RALINK_FRAME | TX_AMSDU_FRAME)) {
				...
				if (NEED_QUEUE_BACK_FOR_AGG(pAd, QueIdx,
					FreeNumber[QueIdx], pTxBlk->TxFrameType)) {
			...
			if (pTxBlk->TxFrameType == TX_RALINK_FRAME ||
				...
				do {
					...
					/* For TX_AMSDU_FRAME/TX_RALINK_FRAME,
					*  Need to check if next pakcet can do aggregation.
					*/
					pPacket = QUEUE_ENTRY_TO_PACKET(pEntry);
				...
				IN PUCHAR pData, IN ULONG DataSize, IN UCHAR OpMode)
{
	...
	if ((FromWhichBSSID < pAd->ApCfg.BssidNum)
		...
		VLAN_Size =
			(pAd->ApCfg.MBSSID[FromWhichBSSID].wdev.VLAN_VID != 0) ?
			LENGTH_802_1Q : 0;
	...
	while (DataSize > LENGTH_802_3) {
		...
		if ((Header802_3[12] == 0x88) && (Header802_3[13] == 0x8E)
			...
			if (Elem != NULL) {
				memcpy(Elem->Msg + (LENGTH_802_11 + LENGTH_802_1_H),
		...
		if ((Header802_3[12] == 0x88) && (Header802_3[13] == 0x8E)) {
			...
			MLME_QUEUE_ELEM *Elem;
			/* = (MLME_QUEUE_ELEM *)
			* kmalloc(sizeof(MLME_QUEUE_ELEM), MEM_ALLOC_FLAG);
			*/
...
#ifdef CONFIG_AP_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
				AP_ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd,
					pClonePacket, RTMP_GET_PACKET_IF(pPacket));
#endif /* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
/* #ifdef P2P_SUPPORT */
#if defined(P2P_SUPPORT) || defined(RT_CFG80211_P2P_SUPPORT)
			if (OpMode == OPMODE_AP) {
				...
				ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd,
					pClonePacket, RTMP_GET_PACKET_IF(pPacket));
			}
#else
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
...
VOID Indicate_AMSDU_Packet(IN PRTMP_ADAPTER pAd, IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	nMSDU =
	    deaggregate_AMSDU_announce(pAd, pRxBlk, pRxBlk->pRxPacket,
		pRxBlk->pData, pRxBlk->DataSize, pRxBlk->OpMode);
}
...
VOID AssocParmFill(IN PRTMP_ADAPTER pAd,
			...
			IN PUCHAR pAddr, IN USHORT CapabilityInfo,
			   ...
			   IN struct wifi_dev *wdev, OUT UCHAR *pUserPriority, OUT UCHAR *pQueIdx)
{
	...
	case ETH_TYPE_IPv4:
		{
			...
			if (*(pSrcBuf + 9) == IP_PROTO_UDP) {
				...
				if (wdev->wdev_type == WDEV_TYPE_AP) {
					...
					WNMIPv4ProxyARPCheck(pAd, pPacket,
						srcPort, dstPort, pSrcBuf);
#endif /* CONFIG_DOT11V_WNM */
#ifdef CONFIG_HOTSPOT
					{
					UCHAR Wcid = RTMP_GET_PACKET_WCID(pPacket);

					if (!HSIPv4Check(pAd, &Wcid,
						pPacket, pSrcBuf,
						srcPort, dstPort))
						return FALSE;
					}
	...
	case 0x892E:
		{
			...
			if (priority >= 256 && priority <= 263)
				up = priority - 256;
			DBGPRINT(RT_DEBUG_INFO, ("%s:OZMO: priority:%d\n",
				__func__, up));
		}
		break;
	default:
	...
	/* NOTE: AP doesn't have to negotiate TSPEC.
	*  ACM is controlled purely via user setup, not protocol handshaking
	*/
	/*
...
VOID Update_Rssi_Sample(IN RTMP_ADAPTER *pAd, IN RSSI_SAMPLE * pRssi, IN RXWI_STRUC *pRxWI)
{
	...
	if (snr[0] != 0) {
		...
		if (bInitial) {
			...
			pRssi->AvgSnr0X8 = (pRssi->AvgSnr0X8 - pRssi->AvgSnr0) +
				pRssi->LastSnr0;
	...
	if (rssi[0] != 0) {
		...
		if (bInitial) {
			...
			pRssi->AvgRssi0X8 =
				(pRssi->AvgRssi0X8 - pRssi->AvgRssi0) +
				pRssi->LastRssi0;
	...
	if (snr[1] != 0) {
		...
		if (bInitial) {
			...
			pRssi->AvgSnr1X8 = (pRssi->AvgSnr1X8 - pRssi->AvgSnr1) +
				pRssi->LastSnr1;
...
/* +++Add by shiang for debug */
	if (0)
		hex_dump("After80211_2_8023",
			GET_OS_PKT_DATAPTR(pRxBlk->pRxPacket),
		...
		if (pRxBlk->pRxInfo->Bcast && (pAd->ApCfg.bMACRepeaterEn)
			...
			if (*(pLayerHdr + 9) == 0x11) {
				...
				if (srcPort == 67 && dstPort == 68) {	/*It's a DHCP packet */
					...
					pReptEntry =
						RTMPLookupRepeaterCliEntry(pAd,
						FALSE, pCliHwAddr);
					if (pReptEntry)
...
VOID CmmRxRalinkFrameIndicate(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry,
			      IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
	UCHAR Header802_3[LENGTH_802_3];
	UINT16 Msdu2Size;
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	    pPacket2 = duplicate_pkt(get_netdev_from_bssid(pAd, FromWhichBSSID),
					...
					pData2, Payload2Size, FromWhichBSSID);
#endif /* CONFIG_STA_SUPPORT */
...
PNDIS_PACKET RTMPDeFragmentDataFrame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (pHeader->Frag == 0) {	/* Frag. Number is 0 : First frag or only one pkt */
		...
		/* Broadcom AP(BCM94704AGR) will send out LLC in fragment's packet,
		* LLC only can accpet at first fragment.
		*/
	...
	if (bReassDone) {
		...
		if (pNewFragPacket) {
			...
			/* Fix MT5396 crash issue when Rx fragmentation frame for
			*  Wi-Fi TGn 5.2.4 & 5.2.13 test items.
			*/
...
VOID RtmpEnqueueNullFrame(IN RTMP_ADAPTER *pAd, IN UCHAR *pAddr,
			...
			IN BOOLEAN bQosNull, IN BOOLEAN bEOSP, IN UCHAR OldUP)
{
	NDIS_STATUS NState;
				...
				UCHAR Index)
{
	...
	if (NState == NDIS_STATUS_SUCCESS) {
		...
		pNullFr->Duration = pAd->CommonCfg.Dsifs +
			RTMPCalcDuration(pAd, pAd->CommonCfg.TxRate, 14);
		...
		DBGPRINT(RT_DEBUG_TRACE,
			...
			Index, pAd->NullBufOffset[Index]));

		for (i = 0; i < Length; i += 4) {
			longValue = *ptr + (*(ptr + 1) << 8) +
				(*(ptr + 2) << 16) + (*(ptr + 3) << 24);
			if (Index == 0)	/* for ra0 */
				RTMP_IO_WRITE32(pAd, pAd->chipCap.BcnBase[14] +
				TXWISize + i, longValue);
			else if (Index == 1)	/* for p2p0 */
				RTMP_IO_WRITE32(pAd, pAd->chipCap.BcnBase[15] +
				TXWISize + i, longValue);
...
VOID dev_rx_ctrl_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	case SUBTYPE_BLOCK_ACK_REQ:
		CntlEnqueueForRecv(pAd, pRxBlk->wcid,
			(pRxBlk->MPDUtotalByteCnt), (PFRAME_BA_REQ) pHeader);
		{
	...
	case SUBTYPE_RTS:
		if (pAd->CommonCfg.vht_bw_signal && pRxBlk->wcid <= MAX_LEN_OF_MAC_TABLE) {
			...
			if ((rts->Addr1[0] & 0x1) == 0x1) {
				srv_field = (PLCP_SERVICE_FIELD *)
					&pRxBlk->pRxWI->RXWI_N.bbp_rxinfo[15];
...
BOOLEAN rtmp_rx_done_handle(RTMP_ADAPTER *pAd)
{
	...
	while (1) {
		...
		pRxBlk = &rxblk;
		pRxPacket = GetPacketFromRxRing(pAd, pRxBlk, &bReschedule,
			&RxPending, &bCmdRspPacket, 0);
...
#ifdef RT_CFG80211_AP_SUPPORT
		if (CFG_AP_ON(pAd) && pAd->cfg80211_ctrl.ap_mac_is_changed &&
			NdisEqualMemory(pAd->cfg80211_ctrl.APCurrentAddress,
...
/* ---Add by shiang for debug */
		...
		else if (RTMP_CFG80211_MultiApIf_ON(pAd) &&
			(NdisEqualMemory(pAd->cfg80211_ctrl.multi_ap_net_dev->dev_addr,
			pHeader->Addr1, MAC_ADDR_LEN) ||
			(pHeader->FC.SubType == SUBTYPE_PROBE_REQ)))
			SET_PKT_OPMODE_AP(&rxblk);
#endif
#ifdef RT_CFG80211_AP_SUPPORT
		else if (CFG_AP_ON(pAd) &&
			 NdisEqualMemory(pAd->cfg80211_ctrl.APCurrentAddress,
				pHeader->Addr1, MAC_ADDR_LEN)) {
			SET_PKT_OPMODE_AP(&rxblk);
		}
		...
		if (ATE_ON(pAd)) {
			...
			if ((pAd->chipCap.FlgHwTxBfCap) && (pHeader->FC.Type == FC_TYPE_MGMT)) {
				...
				if ((((pHeader->FC.SubType == SUBTYPE_ACTION) ||
					(pHeader->FC.SubType == SUBTYPE_ACTION_NO_ACK)) &&
					/*non-compressed beamforming report */
					(pRxBlk->pData)[0] == CATEGORY_HT &&
					((pRxBlk->pData)[1] == MIMO_N_BEACONFORM ||
					(pRxBlk->pData)[1] == MIMO_BEACONFORM))) {
		...
		if (MONITOR_ON(pAd) && pAd->monitor_ctrl.current_monitor_mode != MONITOR_MODE_OFF) {
			...
			if (pAd->monitor_ctrl.current_monitor_mode == MONITOR_MODE_REGULAR_RX) {
				...
				if ((pHeader->FC.Type == FC_TYPE_MGMT)
					...
					pClonePacket =
						ClonePacket(pAd, rxblk.pRxPacket,
						rxblk.pData, rxblk.DataSize);
				...
				pTmpRxPacket = rxblk.pRxPacket;
				pClonePacket = ClonePacket(pAd, rxblk.pRxPacket,
					rxblk.pData, rxblk.DataSize);
				rxblk.pRxPacket = pClonePacket;
				STA_MonPktSend(pAd, &rxblk);
		...
		if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1)) {
			...
			if (MAC_ADDR_EQUAL(pHeader->Addr1, pAd->CurrentAddress))
				pEntry = FindWdsEntry(pAd, pRxBlk->wcid,
					pHeader->Addr2, pRxBlk->rx_rate.field.MODE);

#ifdef STATS_COUNT_SUPPORT
			if (pEntry) {
				pAd->WdsTab.WdsEntry[pEntry->wdev_idx].WdsCounter.ReceivedByteCount
					+= pRxBlk->MPDUtotalByteCnt;
				INC_COUNTER64(pAd->WdsTab.WdsEntry[pEntry->wdev_idx].
					WdsCounter.ReceivedFragmentCount);
					...
					if (MAC_ADDR_EQUAL(pHeader->Addr1, pAd->CurrentAddress))
						pEntry =
						    FindWdsEntry(pAd, pRxBlk->wcid, pHeader->Addr2,
								 pRxBlk->rx_rate.field.MODE);
					if (pEntry)
						pAd->WdsTab.WdsEntry[pEntry->wdev_idx].
							WdsCounter.RxErrors++;
				}
#endif /* STATS_COUNT_SUPPORT */
#endif /* WDS_SUPPORT */
		...
		/* TODO: shiang-usw, for P2P, we original has following code,
		*  need to check it and merge to correct place!!!
		*/
		...
		case FC_TYPE_DATA:
			...
			if (rxblk.bHdrRxTrans) {
				rxblk.bHdrVlanTaged = pRxInfo->tcp_sum_err;	/* RXINFO bit 30 */
				/* 36 byte - RX WIFI Size ( 802.11 Header ) */
				rxblk.pTransData = (UCHAR *) pHeader + 36;
				rxblk.TransDataSize = pRxBlk->MPDUtotalByteCnt;
				rxblk.DataSize += 36;
...
VOID drop_mask_init_per_client(PRTMP_ADAPTER ad, PMAC_TABLE_ENTRY entry)
{
	...
	RTMPInitTimer(ad, &entry->dropmask_timer,
		GET_TIMER_FUNCTION(drop_mask_timer_action), entry, FALSE);
