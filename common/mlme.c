...
CHAR RssiSafeLevelForTxRate[] = { -92, -91, -90, -87, -88, -86, -85,
	-83, -81, -78, -72, -71, -40, -40 };
	...
	do {
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			RTMPInitTimer(pAd, &pAd->Mlme.AutoWakeupTimer,
				GET_TIMER_FUNCTION(RtmpUsbStaAsicForceWakeupTimeout), pAd, FALSE);
...
/*	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);*/
	...
	if (pAd->Mlme.PeriodicRound % MLME_TASK_EXEC_MULTIPLE == 0) {
		...
		fRTMP_ADAPTER_DISABLE_DEQUEUEPACKET) == FALSE)) {
			...
			else if (pApCliEntry->Hostapd != Hostapd_Diable) {
		...
		*/
		...
		(pAd->ApCfg.PartialScanChannelNum == DEFLAUT_PARTIAL_SCAN_CH_NUM)) {
			if (((pAd->ApCfg.PartialScanBreakTime++) %
				...
				/* To check: when EXT_BUILD_CHANNEL_LIST, is the
				*  ScanType switched to SCAN_PASSIVE for DFS channels?
				*/
	...
	if (pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP == WPA_SUPPLICANT_DISABLE)
#endif /* WPA_SUPPLICANT_SUPPORT */
		{
			...
			if (pAd->StaCfg.bBlockAssoc &&
			    RTMP_TIME_AFTER(pAd->Mlme.Now32, pAd->StaCfg.LastMicErrorTime +
				(60 * OS_HZ)))
...
/* 1x1 device for 802.11n WMM Test*/
		...
		if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))) {
			if (pAd->StaCfg.UapsdInfo.bAPSDCapable &&
				...
				/* Send out a NULL frame every 10 sec to inform AP that
				*  STA is still alive (Avoid being age out)
				*/
	...
	IN PMAC_TABLE_ENTRY pMacEntry, IN ULONG Now32)
{
...
#endif /* CONFIG_STA_SUPPORT */
	{
		...
		/* ChannelQuality = W1*RSSI + W2*TxPRR + W3*RxPER (RSSI 0..100),
		(TxPER 100..0), (RxPER 100..0) */
			...
			IN USHORT LengthVIE, IN PNDIS_802_11_VARIABLE_IEs pVIE)
{
	...
	if (Rssi < 0) {
		if (!pBss->bNot1stForEvyScan) {
			...
			if (Rssi > pBss->MaxRssi)
				pBss->MaxRssi = Rssi;
		}
	 }
	 pBss->Rssi = pBss->MaxRssi;
#else
	pBss->Rssi = Rssi;
#endif
		...
		if (ie_list->vht_op_len) {
			...
			if ((vht_op->vht_op_info.ch_width > 0) &&
				...
				if (INFRA_ON(pAd) && CFG80211DRV_OpsScanRunning(pAd)) {
					/* DBGPRINT(RT_DEBUG_TRACE, ("%s:Infra-ON and doing scan,
					* donot update vht_cent_ch\n", __FUNCTION__));
					*/
				} else
#endif /* RT_CFG80211_SUPPORT */
				{
					cent_ch =
						vht_cent_ch_freq(pAd,
...
#define WPS_AP		0x01
			...
			case IE_WAPI:
				if (NdisEqualMemory(pEid->Octet + 4, WAPI_OUI, 3)) {
					...
					/* 2: ID(1 byte), LEN (1 byte), 8: len of "wapi_ie=" */
					pBss->WapiIE.IELen = (pEid->Len * 2) + 8;
				}
				break;
#endif /* WAPI_SUPPORT */
#ifdef EXT_BUILD_CHANNEL_LIST
...
#ifdef EXT_BUILD_CHANNEL_LIST
			/* If no Country IE exists no Connection will be
			*  established when IEEE80211dClientMode is strict.
			*/
			...
			if (wdev->AuthMode >= Ndis802_11AuthModeWPA) {
				...
				if ((wdev->AuthMode == Ndis802_11AuthModeWPA)
					...
					/* If it's not mixed mode, we should only let
					*  BSS pass with the same encryption
					*/
			...
			/*
			*  Linksys WRT610N WPS AP will change the SSID from linksys to
			*  linksys_WPS_<four random characters> when the Linksys WRT610N is in
			*  the state 'WPS Unconfigured' after set to factory default.
			*/
			...
			IN ULONG MsgLen, IN VOID *Msg, IN UCHAR Signal, IN UCHAR OpMode)
{
...
#endif /* P2P_SUPPORT */
	{
		if (!MsgTypeSubst(pAd, pFrame, &Machine, &MsgType)) {
			DBGPRINT(RT_DEBUG_INFO, ("%s(): un-recongnized mgmt->subtype=%d\n",
 ...
 *	\pre *S		should be a legal pointer to the state machine, st, msg,
 *			should be all within the range, Base should be set in the initial state
 *	\post
	...
	IN UCHAR Wcid, IN HT_CAPABILITY_IE * pHtCap, IN ADD_HT_INFO_IE * pAddHtInfo)
{
	...
	if (pAd->MlmeAux.HtCapability.HtCapInfo.ChannelWidth == BW_40) {
		BOOLEAN ret =
			RTMPCheckChannel(pAd, pAd->MlmeAux.CentralChannel,
				pAd->MlmeAux.Channel);
	...
	if (RTMP_GetCustomizedChannelBw(pAd, pAd->MlmeAux.Channel) == BW_20)
		pAd->MlmeAux.HtCapability.HtCapInfo.ChannelWidth = BW_20;
	else
...
/*
	if (pAd->CommonCfg.bAPSDCapable && pAd->CommonCfg.APEdcaParm.bAPSDCapable
		...
		|| (((pAd->CommonCfg.TriggerTimerCount%20) == 19)
...
UCHAR RTMPStaFixedTxMode(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		...
		if (IS_ENTRY_CLIENT(pEntry))
			tx_mode =
				(UCHAR) pAd->ApCfg.MBSSID[pEntry->apidx].wdev.
...
/*
	Compensate last Beacon receive time.
	It is possible that during some circumstances
	that the Beacon frames are not received.
	For example, during scan, or during wow suspend.
	In order to avoid Beacon lost alarm been falsely
	triggered, we need to compensate
	the last Beacon receive time.
	I.e., Just simulate that Beacon has been received anyway.
*/
VOID MlmeCompensateLastBeaconRxTime(PRTMP_ADAPTER pAd, ULONG Now)
{
