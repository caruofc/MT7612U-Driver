...
BOOLEAN ba_reordering_resource_init(RTMP_ADAPTER *pAd, int num)
{
	...
	os_alloc_mem_suspend(pAd, (PUCHAR *) &mem, (num * sizeof(struct reordering_mpdu)));
...
/* if ((RTMP_TIME_AFTER((unsigned long)Now32, (unsigned long)
* (pBAEntry->LastIndSeqAtTimer+REORDERING_PACKET_TIMEOUT))
* && (pBAEntry->list.qlen > ((pBAEntry->BAWinSize*7)/8)))
* || (RTMP_TIME_AFTER((unsigned long)Now32,
* (unsigned long)(pBAEntry->LastIndSeqAtTimer+(10*REORDERING_PACKET_TIMEOUT)))
	...
	if ((pBAEntry->ORI_BA_Status == Originator_WaitRes)
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			if (INFRA_ON(pAd) &&
				...
				/* In scan progress and have no chance to send out,
				*  just re-schedule to another time period
				*/
		...
		DBGPRINT(RT_DEBUG_TRACE,
			("BA Ori Session Timeout(%d) ADD BA TID %d again\n",
			pBAEntry->Token, pBAEntry->TID));
...
VOID PeerAddBAReqAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
#endif /* CONFIG_STA_SUPPORT */
	{
		if (pMacEntry->wdev == NULL) {
			...
			goto error;
		} else if (pMacEntry->wdev->if_addr == NULL) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s: wdev->if_addr is NULL\n", __func__));
			goto error;
		} else if (pMacEntry->wdev->bssid == NULL) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s: wdev->bssid is NULL\n", __func__));
			goto error;
		}
		if (pAddr == NULL) {
	...
	IN ULONG Wcid, IN ULONG MsgLen, IN PFRAME_BA_REQ pMsg)
{
	...
	if ((Wcid < MAX_LEN_OF_MAC_TABLE) && (TID < 8)) {
		/* if this receiving packet is from SA that is in our OriEntry.
		*  Since WCID <9 has direct mapping. no need search.
		*/
		Idx = pAd->MacTab.Content[Wcid].BARecWcidArray[TID];
		pBAEntry = &pAd->BATable.BARecEntry[Idx];
	} else {
