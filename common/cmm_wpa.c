		...
		if ((pEntry->Sst == SST_ASSOC) && (pEntry->WpaState >= AS_INITPSK)) {
			...
			if (peerKeyInfo.KeyAck == 1) {
				...
				if ((peerKeyInfo.Request == 0) &&
					...
					/* Need to check KeyType for groupkey or
					pairwise key update, refer to 8021i P.114,*/
			     ...
			     IN ULONG MsgLen, IN UCHAR MsgType, IN MAC_TABLE_ENTRY *pEntry)
{
	...
	*((USHORT *) &EapolKeyInfo) = cpu2le16(*((USHORT *) &EapolKeyInfo));
	...
	if (MsgType == EAPOL_PAIR_MSG_1
		...
		|| MsgType == EAPOL_GROUP_MSG_1) {	/* For supplicant */
		...
		if ((RTMPCompareMemory
			...
			|| MsgType == EAPOL_GROUP_MSG_2) {	/* For authenticator */
...
#ifdef DOT11W_PMF_SUPPORT
	if ((CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_USE_SHA256))) {
		PMF_DerivePTK(pAd, (UCHAR *) pmk_ptr,
			pEntry->ANonce, pEntry->Addr,
			pEntry->SNonce, pCurrentAddr, PTK,
			...
			IN MAC_TABLE_ENTRY *pEntry, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if ((CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_USE_SHA256))) {
		PMF_DerivePTK(pAd, (UCHAR *) pmk_ptr,
			pEntry->ANonce, (UCHAR *) pBssid,
			pEntry->SNonce, pEntry->Addr, PTK,
			LEN_AES_PTK);	/* Must is 48 bytes */
	      ...
	      unsigned char *output)
{
	...
	RT_HMAC_SHA1((unsigned char *)password, len,
		digest, ssidlength + 4, digest1,
		SHA1_DIGEST_SIZE);	/* for WPA update */
	...
	for (i = 1; i < iterations; i++) {
		/* Un = PRF(P, Un-1) */
		RT_HMAC_SHA1((unsigned char *)password, len,
		digest1, SHA1_DIGEST_SIZE, digest,
		SHA1_DIGEST_SIZE);	/* for WPA update */
...
PSTRING GetEapolMsgType(CHAR msg)
{
	...
	if (msg == EAPOL_PAIR_MSG_2)
		return "Pairwise Message 2";

	if (msg == EAPOL_PAIR_MSG_3)
		return "Pairwise Message 3";

	if (msg == EAPOL_PAIR_MSG_4)
		return "Pairwise Message 4";

	if (msg == EAPOL_GROUP_MSG_1)
		return "Group Message 1";

	if (msg == EAPOL_GROUP_MSG_2)
...
#endif /* endif */
			ret = RTMPSoftDecryptSMS4(pHdr, FALSE, pKey, pData,
						 &(*DataByteCnt));
...
/* If the received frame is EAP-Packet ,find out its EAP-Code (Request(0x01),
Response(0x02), Success(0x03), Failure(0x04)). */
