	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (pPmfCfg) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			MgtMacHeaderInit(pAd, &SAQReqHdr, SUBTYPE_ACTION, 0,
				pEntry->Addr, pAd->ApCfg.MBSSID[pEntry->apidx].wdev.if_addr,
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (Action == ACTION_SAQ_REQUEST) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			("[PMF]%s - Send SA Query Response to STA(%02x:%02x:%02x:%02x:%02x:%02x)\n",
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (Action == ACTION_SAQ_RESPONSE) {
		...
		if (pEntry->TransactionID == TransactionID) {
			...
			DBGPRINT(RT_DEBUG_ERROR,
				("[PMF]%s - Compare TransactionID correctly, STA(%02x:%02x:%02x:%02x:%02x:%02x)\n",
				__func__, PRINT_MAC(pHeader->Hdr.Addr2)));
		} else {
			DBGPRINT(RT_DEBUG_ERROR,
				("[PMF]%s - Compare TransactionID wrong, STA(%02x:%02x:%02x:%02x:%02x:%02x)\n",
				__func__, PRINT_MAC(pHeader->Hdr.Addr2)));
		}
	}
}
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		if (RTMPGetKeyParameter("PMFMFPC", tmpbuf, 32, pBuffer, TRUE)) {
			for (apidx = 0, macptr = rstrtok(tmpbuf, ";");
				(macptr && apidx < pAd->ApCfg.BssidNum); macptr = rstrtok(NULL, ";"), apidx++) {
				pObj->ioctl_if = apidx;
	...
	PSTRING arg)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("[PMF]%s:: apidx=%d, Desired MFPC=%d\n",
			__func__, pObj->ioctl_if, pAd->ApCfg.MBSSID[pObj->ioctl_if].PmfCfg.Desired_MFPC));
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("[PMF]%s:: MFPC=%d, MFPR=%d, SHA256=%d\n",
					__func__, pAd->StaCfg.PmfCfg.MFPC, pAd->StaCfg.PmfCfg.MFPR,
