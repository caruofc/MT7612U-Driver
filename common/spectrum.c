...
VOID EnqueueChSwAnn(IN PRTMP_ADAPTER pAd, IN PUCHAR pDA, IN UINT8 ChSwMode, IN UINT8 NewCh)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s: ChSwMode=%d, NewCh=%d\n",
		    __func__, ChSwMode, NewCh));
	/* build action frame header. */
	MgtMacHeaderInit(pAd, &ActHdr, SUBTYPE_ACTION, 0, pDA,
			 pAd->CurrentAddress, pAd->CurrentAddress);
	...
	while (((UCHAR *) eid_ptr + eid_ptr->Len + 1) < ((PUCHAR) pFramePtr + MsgLen)) {
		...
		case IE_CHANNEL_SWITCH_ANNOUNCEMENT:
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s - IE_CHANNEL_SWITCH_ANNOUNCEMENT., ChSwMode=%d NewCh=%d ChSwCnt=%d.\n",
				  __func__, pChSwAnnInfo->ChSwMode, pChSwAnnInfo->Channel,
				  pChSwAnnInfo->ChSwCnt));
...
static VOID PeerChSwAnnAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (pAd->OpMode == OPMODE_STA) {
		...
		hex_dump("BSSID", pAd->ScanTab.BssEntry[Bssidx].Bssid, 6);
		...
		if ((pAd->CommonCfg.bIEEE80211H == 1) && (NewChannel != 0)
		    && (Channel != NewChannel)) {
			/* Switching to channel 1 can prevent from rescanning the
			current channel immediately (by auto reconnection). */
			...
			for (index = 0; index < pAd->ChannelListNum; index++) {
				if (pAd->ChannelList[index].Channel == NewChannel) {
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("!!%s: Received CSA IE (New Channel =%d)\n",
						  __func__, NewChannel));
					break;
				}
			}
			...
			if (index >= pAd->ChannelListNum) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("!!%s: Can not find New Channel=%d in ChannelList[%d]\n",
					  __func__, pAd->CommonCfg.Channel, pAd->ChannelListNum));
			}
		}
	}
#endif /* CONFIG_STA_SUPPORT */
...
static VOID PeerTpcReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	/* STA may incorporate rate information and channel conditions, including interference,*/
	/* into its computation of link margin. */
...
VOID PeerSpectrumAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	case SPEC_CHANNEL_SWITCH:

		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("%s: SPEC_CHANNEL_SWITCH state\n", __func__));
			...
			/* Before perform channel switch action,
			 * we will need to check if BSSID is correct.
			 */
			if ((!pAd->CommonCfg.Channel) ||
			    (!MAC_ADDR_EQUAL(pFr->Hdr.Addr3, pAd->CommonCfg.Bssid))) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: Ignore Channel Switch Action from other BSSID\n",
				      __func__));
				break;
			}
...
INT Set_MeasureReq_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
		switch (ArgIdx) {
		case 1:	/* Aid. */
			ret = os_strtol(thisChar, 16, &longtmp);
			if (ret < 0)
				return FALSE;
			wcid = (UINT8) longtmp;
			break;
