...
#ifdef AGS_SUPPORT
if ((bUseAGS == TRUE) && (pCurrTxRate->CurrMCS >= 16) && (pCurrTxRate->CurrMCS <= 23)) {
					switch (pCurrTxRate->CurrMCS) {
					case 16:
...
/*
	...
	Routine Description:
		...
		= 0x01; there may be non-HT devices in both the control and extension channel, protection is optional
		in BSS.
	...
	if ((SetMask & ALLN_SETPROTECT) != 0) {
		...
		case 1:
			...
			if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_BG_PROTECTION_INUSED)) {
				/*ERP use Protection bit is set, use protection rate at Clause 18.. */
...
#ifdef TXBF_SUPPORT
	/* FW will initialize TxBf HW status. Re-calling this AP could recover previous status */
	rtmp_asic_set_bf(pAd);

#ifdef MT76x2
	if (IS_MT76x2(pAd)) {
		/* Disable BF HW to apply profile to packets when nSS == 2. */
		/* Maybe it can be initialized at chip init but removing the same CR initialization from FW will be
		   better */
...
VOID AsicSetBssid(RTMP_ADAPTER *pAd, UCHAR *pBssid)
{
	...
	Addr4 |= (UINT32)pBssid[1] << 8;
	Addr4 |= (UINT32)pBssid[2] << 16;
	Addr4 |= (UINT32)pBssid[3] << 24;
	...
	Addr4 = (UINT32)pBssid[4];
	Addr4 |= (UINT32)pBssid[5] << 8;
	...
	if (CFG80211_P2P_TEST_BIT(pAd->cfg80211_ctrl.P2POpStatusFlags, CFG_P2P_IF_UP))
		enableMBSSIDMode = TRUE;
#endif

#ifdef RT_CFG80211_AP_SUPPORT
	if (pAd->cfg80211_ctrl.cfg_ap_is_working)
		enableMBSSIDMode = TRUE;
	...
	if (enableMBSSIDMode) {
		...
		Addr4 |= (UINT32)pP2PBssid[1] << 8;
		Addr4 |= (UINT32)pP2PBssid[2] << 16;
		Addr4 |= (UINT32)pP2PBssid[3] << 24;
		RTMP_IO_WRITE32(pAd, MAC_BSSID_DW0, Addr4);
		...
		/* always one BSSID in STA mode */
		Addr4 = (UINT32)pP2PBssid[4];
		Addr4 |= (UINT32)pP2PBssid[5] << 8;
		RTMP_IO_WRITE32(pAd, MAC_BSSID_DW1, Addr4);
...
INT AsicSetRepeaterMode(PRTMP_ADAPTER pAd, BOOLEAN bEnable)
{
	...
	if (bEnable) {
		MacReg |= 0x1;
		DBGPRINT(RT_DEBUG_OFF, (" Repeater Mode (ON)\n"));
	} else {
		MacReg &= (~0x1);
		DBGPRINT(RT_DEBUG_OFF, (" Repeate Mode (OFF)\n"));
	}
...
VOID AsicSetRepeaterExtAddr(PRTMP_ADAPTER pAd, PUCHAR pBssid)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("%s():%x:%x:%x:%x:%x:%x\n",
	...
	Addr4 = (UINT32)(pBssid[0]) |
			...
			(UINT32)(pBssid[2] << 16) |
			(UINT32)(pBssid[3] << 24);
	RTMP_IO_WRITE32(pAd, MAC_ADDR_EXT0_31_0, Addr4);

	Addr4 = 0;
	Addr4 = (UINT32)(pBssid[4]) | (UINT32)(pBssid[5] << 8);

	RTMP_IO_WRITE32(pAd, MAC_ADDR_EXT0_47_32, Addr4);
}
...
INT AsicSetRxFilter(RTMP_ADAPTER *pAd)
{
	...
	if ((MONITOR_ON(pAd)) && pAd->monitor_ctrl.current_monitor_mode == MONITOR_MODE_FULL)
		rx_filter_flag = 0x3;
#endif /* endif */
...
VOID AsicEnableApBssSync(IN PRTMP_ADAPTER pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("--->AsicEnableBssSync(AP mode)\n"));
...
/*
    ==================================================
    Routine Description:
	Add some compensation values to TSF before HW fill the TSF value into Beacon
	timestamp field
    Arguments:
	pAd - Adapter pointer
	val  - TSF compensation value (unit:us)
...
VOID AsicSetTxTSFCompensation(PRTMP_ADAPTER pAd, u32 val)
{
	...
	RTMP_IO_READ32(pAd, BCN_TIME_CFG, &csr.word);
	csr.field.TxTimestampCompensate = val;
	RTMP_IO_WRITE32(pAd, BCN_TIME_CFG, csr.word);
}
...
VOID AsicSetEdcaParm(RTMP_ADAPTER *pAd, PEDCA_PARM pEdcaParm)
{
	...
	if ((pEdcaParm == NULL) || (pEdcaParm->bValid == FALSE)) {
		...
		if (SIGMA_ON(pAd)) {
			...
			Ac2Cfg.field.Cwmin += 1;
			Ac2Cfg.field.Cwmax += 1;
...
#ifdef CONFIG_STA_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			/*pEdcaParm->Aifsn[QID_AC_VO]; for TGn wifi test */
...
/* return number of entry updated in the input array */
UINT32 AsicReadAggCnt(RTMP_ADAPTER *pAd, UINT32 *aggCnt, int size)
{
	UINT32 reg, cnt, i;
	TX_AGG_CNT_STRUC val;
	static USHORT aggReg[] = {
		TX_AGG_CNT0, TX_AGG_CNT3,
#if MAX_AGG_CNT > 8
	...
	if (size < 2) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s():too small size %d <2\n",
			 __func__, size));
		return 0;
	}

	NdisZeroMemory(aggCnt, size * sizeof(typeof(aggCnt[0])));

	cnt = 2;
	for (i = 0; (i + 1) < ARRAY_SIZE(aggReg); i += 2) {
		for (reg = aggReg[i]; reg <= aggReg[i + 1]; reg += 4) {
			RTMP_IO_READ32(pAd, reg, &val.word);
			if (cnt < (size - 1)) {
				aggCnt[cnt] = val.field.cnt0_15;
				aggCnt[cnt + 1] = val.field.cnt16_31;
				DBGPRINT(RT_DEBUG_LOUD,
					 ("%s():Get AggSize reg 0x%x val 0x%08x AGG_%d %d, AGG_%d %d\n",
					  __func__, reg, val.word,
					  cnt - 1, aggCnt[cnt],
					  cnt, aggCnt[cnt + 1]));
				cnt += 2;
			} else
				DBGPRINT(RT_DEBUG_WARN,
					 ("%s():Get AggSize reg 0x%x val 0x%08x buffer size %d now cnt %d\n",
					  __func__, reg, val.word, size, cnt));
		}
	}

	/* get total after read done each */
	RTMP_IO_READ32(pAd, TX_NAG_AGG_CNT, &val.word);
	aggCnt[0] = val.field.cnt0_15;
	aggCnt[1] = val.field.cnt16_31;
	DBGPRINT(RT_DEBUG_INFO,
		 ("%s():Get AggSize reg 0x%x val 0x%08x NAG %d AGG %d\n",
		  __func__, TX_NAG_AGG_CNT, val.word, aggCnt[0], aggCnt[1]));
...
VOID RT28xxAndesWOWEnable(IN PRTMP_ADAPTER pAd)
{
	...
	if (Ret != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR, ("\x1b[31m%s: send WOW config command failed(%d/%d)!!\x1b[m\n", __func__,
	...
	if (pAd->StaCfg.AuthMode >= Ndis802_11AuthModeWPAPSK) {
		...
		DBGPRINT(RT_DEBUG_INFO, ("\x1b[31m%s: wep status %d\x1b[m\n", __func__, pEntry->WepStatus));
		sec_cfg.Group_Cipher = pAd->StaCfg.GroupCipher;
		DBGPRINT(RT_DEBUG_INFO, ("\x1b[31m%s: group status %d\x1b[m\n", __func__, sec_cfg.Group_Cipher));
		DBGPRINT(RT_DEBUG_INFO, ("\x1b[31m%s: aid %d\x1b[m\n", __func__, pEntry->Aid));
	...
	if (pAd->WOW_Cfg.bInBand) {
		...
		DBGPRINT(RT_DEBUG_INFO, ("\x1b[31m%s: 0x80 = %x\x1b[m\n", __func__, Value));
...
VOID RT28xxAndesWOWDisable(IN PRTMP_ADAPTER pAd)
{
	...
	DBGPRINT(RT_DEBUG_INFO, ("\x1b[31m%s: ...\x1b[m", __func__));
	...
	if (Ret != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR, ("\x1b[31m%s: send WOW config command failed!!\x1b[m\n", __func__));
		return;
	}
	...
	RTMP_IO_READ32(pAd, WLAN_FUN_CTRL, &Value);
	DBGPRINT(RT_DEBUG_INFO, ("\x1b[31m%s: 0x80 %08x\x1b[m\n", __func__, Value));
	Value &= ~0x80;
	RTMP_IO_WRITE32(pAd, WLAN_FUN_CTRL, Value);
...
VOID thermal_protection(IN RTMP_ADAPTER *pAd)
{
	...
	if (!pAd->chipCap.ThermalProtectSup || !IS_MT76x2(pAd))
		return;

#ifdef NEW_WOW_SUPPORT
	if ((pAd->WOW_Cfg.bInSuspendMode != WOW_NOT_IN_SUSPEND)
		...
		DBGPRINT(RT_DEBUG_ERROR, ("%s Resume is not completed just return!\n", __func__));
		return;
	}
#endif /* NEW_WOW_SUPPORT */

	/* If MT76x2U go into suspend mode, thermal clock will also be disabled.
	 * After resume, MCU will hang if driver retrieve thermal value without calibration.
	 */
	RTMP_IO_READ32(pAd, WLAN_FUN_CTRL, &value);
	if ((value & WLAN_FUN_CTRL_THERM_CKEN) != WLAN_FUN_CTRL_THERM_CKEN) {
		DBGPRINT(RT_DEBUG_ERROR,
			("%s: WLAN_FUN_CTRL = 0x%x thermal clock is not enabled, do calibration!\n",
			__func__, value));
	...
	DBGPRINT(RT_DEBUG_INFO, ("%s - current temp=%d\n", __func__, current_temp));

	/* 2T2R to 1T2R */
	if (pAd->thermal_HighEn) {
		if ((current_temp > pAd->thermal_HighTempTh) && !pAd->force_one_tx_stream) {
			if (pAd->set_one_tx_stream == FALSE)
				pAd->switch_tx_stream = TRUE;
			pAd->set_one_tx_stream = TRUE;

			/* 0x504: Turn on BIT[13][14] */
			RTMP_IO_READ32(pAd, RLT_RF_BYPASS_0, &mac_reg);
			mac_reg |= (3 << 13);
			RTMP_IO_WRITE32(pAd, RLT_RF_BYPASS_0, mac_reg);

			/* 0x50C: Turn off BIT[13][14] */
			RTMP_IO_READ32(pAd, RLT_RF_SETTING_0, &mac_reg);
			mac_reg &= ~(3 << 13);
			RTMP_IO_WRITE32(pAd, RLT_RF_SETTING_0, mac_reg);

			DBGPRINT(RT_DEBUG_OFF,
				("%s - current temp=%d > HighTempTh =%d switch to 1T\n",
				__func__, current_temp, pAd->thermal_HighTempTh));
		}
	}

	/* 1T2R to 2T2R */
	if (pAd->thermal_LowEn) {
		if ((current_temp < pAd->thermal_LowTempTh) && pAd->force_one_tx_stream) {
			if (pAd->set_one_tx_stream)
				pAd->switch_tx_stream = TRUE;
			pAd->set_one_tx_stream = FALSE;

			/* 0x504: Turn off BIT[13][14] */
			RTMP_IO_READ32(pAd, RLT_RF_BYPASS_0, &mac_reg);
			mac_reg &= ~(3 << 13);
			RTMP_IO_WRITE32(pAd, RLT_RF_BYPASS_0, mac_reg);
			...
			DBGPRINT(RT_DEBUG_OFF,
				("%s - current temp=%d < LowTempTh =%d restore to 2T\n",
				__func__, current_temp, pAd->thermal_LowTempTh));
		}
	}
