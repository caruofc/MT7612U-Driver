...
/* GeK: mt7662t is USB only, and can remove compile flag only if we support
 * both USB and PCIE in one driver ko.
 */
...
static void mt76x2_switch_channel(RTMP_ADAPTER *ad, u8 channel, BOOLEAN scan)
{
	...
	/* enable TX/RX */
	RTMP_IO_WRITE32(ad, 0x1004, 0xC | restore_value);
...
UCHAR mt76x2_update_sku_pwr_proc(RTMP_ADAPTER *ad, CHAR ch_delta_pwr, CH_POWER *ch)
{
	INT32 i = 0;
	INT32 pwr_delta = 0;
	INT32 rate_pwr = 0;
		...
		if (channel >= start_ch) {
			for (j = 0; j < ch->num; j++) {
				if (channel == ch->Channel[j]) {
					...
					mt76x2_update_sku_pwr_proc(ad, channel, ch_delta_pwr, ch);
					bFound = TRUE;
					break;
				}
			}
